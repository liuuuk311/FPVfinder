...
<div class="flex flex-row justify-around md:justify-start items-center mb-2">
	...
	<form  on:submit={search} class="flex-grow">
		<div class="flex-grow items-center bg-white rounded-full overflow-hidden">
			<input bind:value={searchedKeywords} class="text-sm md:text-base text-gray-400 flex-grow outline-none py-2 mx-2 px-2 md:w-5/6" type="text" placeholder="{$_('search_placeholder')}" />
			<button on:click={search} class="bg-indigo-500 text-white text-sm md:text-base rounded-full py-2 px-5 font-bold float-right">{$_('search_btn')}</button>
		</div>
	</form>
</div>
